...
import io.attic.presto.noms.DatasetLoader;
import io.attic.presto.noms.util.NomsThriftServer;
...
public final class NomsThriftQueryRunner
{
    private NomsThriftQueryRunner() {}

    public static QueryRunner create(int workers)
            throws Exception
    {
        List<NomsThriftServer> servers = ImmutableList.of(NomsThriftServer.start(DatasetLoader.dbPefix()));
        DistributedQueryRunner runner = null;
        try {
            runner = createInternal(servers, workers);
            ...
            for (NomsThriftServer server : servers) {
                server.close();
            }
            throw t;
        }
    ...
    public static void main(String[] args)
            throws Exception
    {
        ThriftQueryRunnerWithServers queryRunner = (ThriftQueryRunnerWithServers) create(3);
        Thread.sleep(10);
        Logger log = Logger.get(NomsThriftQueryRunner.class);
        log.info("======== SERVER STARTED ========");
        log.info("\n====\n%s\n====", queryRunner.getCoordinator().getBaseUrl());
    }

    private static DistributedQueryRunner createInternal(List<NomsThriftServer> servers, int workers)
            throws Exception
    {
        List<HostAddress> addresses = servers.stream()
                .map(server -> HostAddress.fromParts("localhost", server.uri().getPort()))
            ...
            implements QueryRunner
    {
        ...
        private final List<NomsThriftServer> thriftServers;

        private ThriftQueryRunnerWithServers(DistributedQueryRunner source, List<NomsThriftServer> thriftServers)
        {
        ...
        public void close()
        {
            ...
            for (NomsThriftServer server : thriftServers) {
                server.close();
            }
        }
