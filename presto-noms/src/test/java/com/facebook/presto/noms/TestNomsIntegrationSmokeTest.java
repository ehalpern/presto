...
import static com.facebook.presto.noms.NomsQueryRunner.createCassandraSession;
import static com.facebook.presto.noms.NomsTestingUtils.TABLE_ALL_TYPES;
import static com.facebook.presto.noms.NomsTestingUtils.TABLE_ALL_TYPES_INSERT;
import static com.facebook.presto.noms.NomsTestingUtils.TABLE_ALL_TYPES_PARTITION_KEY;
import static com.facebook.presto.noms.NomsTestingUtils.TABLE_CLUSTERING_KEYS;
import static com.facebook.presto.noms.NomsTestingUtils.TABLE_CLUSTERING_KEYS_INEQUALITY;
import static com.facebook.presto.noms.NomsTestingUtils.TABLE_CLUSTERING_KEYS_LARGE;
import static com.facebook.presto.noms.NomsTestingUtils.TABLE_MULTI_PARTITION_CLUSTERING_KEYS;
import static com.facebook.presto.noms.NomsTestingUtils.createTestTables;
        ...
        extends AbstractTestIntegrationSmokeTest
{
    ...
    public TestNomsIntegrationSmokeTest()
            throws Exception
    {
        super(NomsQueryRunner::createCassandraQueryRunner);
    }
            ...
            throws Exception
    {
        ...
        assertContainsEventually(() -> execute("SHOW SCHEMAS FROM noms"), resultBuilder(getSession(), createUnboundedVarcharType())
                .row("keyspace_1")
                .build(), new Duration(1, MINUTES));

        session.execute("CREATE TABLE KEYSPACE_1.TABLE_1 (COLUMN_1 bigint PRIMARY KEY)");
        assertContainsEventually(() -> execute("SHOW TABLES FROM noms.keyspace_1"), resultBuilder(getSession(), createUnboundedVarcharType())
                .row("table_1")
                .build(), new Duration(1, MINUTES));
        assertContains(execute("SHOW COLUMNS FROM noms.keyspace_1.table_1"), resultBuilder(getSession(), createUnboundedVarcharType(), createUnboundedVarcharType(), createUnboundedVarcharType(), createUnboundedVarcharType())
                .row("column_1", "bigint", "", "")
                .build());

        execute("INSERT INTO keyspace_1.table_1 (column_1) VALUES (1)");

        assertEquals(execute("SELECT column_1 FROM noms.keyspace_1.table_1").getRowCount(), 1);
            ...
            throws Exception
    {
        ...
        assertContainsEventually(() -> execute("SHOW TABLES FROM noms.keyspace_2"), resultBuilder(getSession(), createUnboundedVarcharType())
                .row("table_2")
                .build(), new Duration(1, MINUTES));
        assertContains(execute("SHOW COLUMNS FROM noms.keyspace_2.table_2"), resultBuilder(getSession(), createUnboundedVarcharType(), createUnboundedVarcharType(), createUnboundedVarcharType(), createUnboundedVarcharType())
                .row("column_2", "bigint", "", "")
                .build());

        execute("INSERT INTO \"KEYSPACE_2\".\"TABLE_2\" (\"COLUMN_2\") VALUES (1)");

        assertEquals(execute("SELECT column_2 FROM noms.keyspace_2.table_2").getRowCount(), 1);
            ...
            throws Exception
    {
        ...
        // Although in Presto all the schema and table names are always displayed as lowercase
        assertContainsEventually(() -> execute("SHOW TABLES FROM noms.keyspace_4"), resultBuilder(getSession(), createUnboundedVarcharType())
                .row("table_4")
                .row("table_4")
                .build(), new Duration(1, MINUTES));
            ...
            throws Exception
    {
        ...
        assertContainsEventually(() -> execute("SHOW TABLES FROM noms.keyspace_5"), resultBuilder(getSession(), createUnboundedVarcharType())
                .row("table_5")
                .build(), new Duration(1, MINUTES));
