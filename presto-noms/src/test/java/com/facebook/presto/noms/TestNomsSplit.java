...
public class TestNomsSplit
{
    private final NomsSplit split = new NomsSplit("connectorId", "schemaName", "tableName", URI.create("http://127.0.0.1/test.file"));

    @Test
    public void testAddresses()
    {
        // http split with default port
        NomsSplit httpSplit = new NomsSplit("connectorId", "schemaName", "tableName", URI.create("http://example.com/example"));
        ...
        httpSplit = new NomsSplit("connectorId", "schemaName", "tableName", URI.create("http://example.com:8080/example"));
        assertEquals(httpSplit.getAddresses(), ImmutableList.of(HostAddress.fromParts("example.com", 8080)));
        assertEquals(httpSplit.isRemotelyAccessible(), true);

        // http split with default port
        NomsSplit httpsSplit = new NomsSplit("connectorId", "schemaName", "tableName", URI.create("https://example.com/example"));
        ...
        // http split with custom port
        httpsSplit = new NomsSplit("connectorId", "schemaName", "tableName", URI.create("https://example.com:8443/example"));
    ...
    {
        JsonCodec<NomsSplit> codec = jsonCodec(NomsSplit.class);
        String json = codec.toJson(split);
        NomsSplit copy = codec.fromJson(json);
