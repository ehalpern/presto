...
public class NomsPageSourceProvider
        implements ConnectorPageSourceProvider
{
    private final NomsSession session;

    @Inject
    public NomsPageSourceProvider(
            NomsSession session)
    {
    ...
    {
        return new NomsPageSource(this.session, (NomsSplit) split,
                columns.stream().map(h -> (NomsColumnHandle) h).collect(Collectors.toList()));
    }
}
