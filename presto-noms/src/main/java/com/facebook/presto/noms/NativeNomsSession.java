...
import static com.facebook.presto.noms.NomsErrorCode.CASSANDRA_VERSION_ERROR;
        ...
        implements NomsSession
{
    ...
    public NativeNomsSession(String connectorId, JsonCodec<List<ExtraColumnMetadata>> extraColumnMetadataCodec, Cluster cluster, Duration noHostAvailableRetryTimeout)
    {
            ...
            throws TableNotFoundException
    {
        ...
        for (ColumnMetadata columnMeta : tableMeta.getPartitionKey()) {
            ...
            NomsColumnHandle columnHandle = buildColumnHandle(tableMeta, columnMeta, true, false, columnNames.indexOf(columnMeta.getName()), hidden);
        ...
        for (ColumnMetadata columnMeta : tableMeta.getClusteringColumns()) {
            primaryKeySet.add(columnMeta.getName());
            boolean hidden = hiddenColumns.contains(columnMeta.getName());
            NomsColumnHandle columnHandle = buildColumnHandle(tableMeta, columnMeta, false, true, columnNames.indexOf(columnMeta.getName()), hidden);
            columnHandles.add(columnHandle);
        }
        ...
        for (ColumnMetadata columnMeta : columns) {
            if (!primaryKeySet.contains(columnMeta.getName())) {
                boolean hidden = hiddenColumns.contains(columnMeta.getName());
                NomsColumnHandle columnHandle = buildColumnHandle(tableMeta, columnMeta, false, false, columnNames.indexOf(columnMeta.getName()), hidden);
                columnHandles.add(columnHandle);
            }
        }

        List<NomsColumnHandle> sortedColumnHandles = columnHandles.build().stream()
                .sorted(comparing(NomsColumnHandle::getOrdinalPosition))
                .collect(toList());

        NomsTableHandle tableHandle = new NomsTableHandle(connectorId, tableMeta.getKeyspace().getName(), tableMeta.getName());
        return new NomsTable(tableHandle, sortedColumnHandles);
    }
    ...
    private NomsColumnHandle buildColumnHandle(TableMetadata tableMetadata, ColumnMetadata columnMeta, boolean partitionKey, boolean clusteringKey, int ordinalPosition, boolean hidden)
    {
        NomsType nomsType = NomsType.getCassandraType(columnMeta.getType().getName());
        List<NomsType> typeArguments = null;
        if (nomsType != null && nomsType.getTypeArgumentSize() > 0) {
            List<DataType> typeArgs = columnMeta.getType().getTypeArguments();
            switch (nomsType.getTypeArgumentSize()) {
                case 1:
                    typeArguments = ImmutableList.of(NomsType.getCassandraType(typeArgs.get(0).getName()));
                    break;
                case 2:
                    typeArguments = ImmutableList.of(NomsType.getCassandraType(typeArgs.get(0).getName()), NomsType.getCassandraType(typeArgs.get(1).getName()));
                    break;
                default:
        ...
        return new NomsColumnHandle(connectorId, columnMeta.getName(), ordinalPosition, nomsType, typeArguments, partitionKey, clusteringKey, indexed, hidden);
    }

    @Override
    public List<NomsPartition> getPartitions(NomsTable table, List<Object> filterPrefix)
    {
        Iterable<Row> rows = queryPartitionKeys(table, filterPrefix);
        if (rows == null) {
            // just split the whole partition range
            return ImmutableList.of(NomsPartition.UNPARTITIONED);
        }

        List<NomsColumnHandle> partitionKeyColumns = table.getPartitionKeyColumns();
        ...
        for (Row row : rows) {
            ...
            for (int i = 0; i < partitionKeyColumns.size(); i++) {
                ...
                NomsColumnHandle columnHandle = partitionKeyColumns.get(i);
                NullableValue keyPart = NomsType.getColumnValueForPartitionKey(row, i, columnHandle.getNomsType(), columnHandle.getTypeArguments());
                ...
                stringBuilder.append(NomsType.getColumnValueForCql(row, i, columnHandle.getNomsType()));
            }
            buffer.flip();
            ...
            if (uniquePartitionIds.add(partitionId)) {
                partitions.add(new NomsPartition(key, partitionId, tupleDomain, false));
            }
        }
    ...
    private Iterable<Row> queryPartitionKeys(NomsTable table, List<Object> filterPrefix)
    {
        NomsTableHandle tableHandle = table.getTableHandle();
        List<NomsColumnHandle> partitionKeyColumns = table.getPartitionKeyColumns();
    ...
    private static void addWhereClause(Where where, List<NomsColumnHandle> partitionKeyColumns, List<Object> filterPrefix)
    {
        for (int i = 0; i < filterPrefix.size(); i++) {
            NomsColumnHandle column = partitionKeyColumns.get(i);
            Object value = column.getNomsType().getJavaValue(filterPrefix.get(i));
