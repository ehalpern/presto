...
import com.facebook.presto.noms.NomsTableHandle;
...
public final class CassandraCqlUtils
{
    ...
    private static void appendSelectColumns(StringBuilder stringBuilder, List<? extends ColumnHandle> columns, boolean first)
    {
        for (ColumnHandle column : columns) {
            ...
            stringBuilder.append(validColumnName(((NomsColumnHandle) column).getName()));
        }
    }
    ...
    public static Selection select(List<NomsColumnHandle> columns)
    {
        Selection selection = QueryBuilder.select();
        for (NomsColumnHandle column : columns) {
            selection.column(validColumnName(column.getName()));
        }
        return selection;
    }

    public static Select selectFrom(NomsTableHandle tableHandle, List<NomsColumnHandle> columns)
    {
        return from(select(columns), tableHandle);
    }

    public static Select from(Selection selection, NomsTableHandle tableHandle)
    {
    ...
    public static Select selectDistinctFrom(NomsTableHandle tableHandle, List<NomsColumnHandle> columns)
    {
        return from(select(columns).distinct(), tableHandle);
    }

    public static Select selectCountAllFrom(NomsTableHandle tableHandle)
    {
        String schema = validSchemaName(tableHandle.getSchemaName());
        String table = validTableName(tableHandle.getTableName());
