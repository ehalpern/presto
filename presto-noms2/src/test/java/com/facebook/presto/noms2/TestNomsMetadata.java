...
public class TestNomsMetadata
{
    private static final String CONNECTOR_ID = "TEST";
    private static final NomsTableHandle NUMBERS_TABLE_HANDLE = new NomsTableHandle(CONNECTOR_ID, "example", "numbers");
            ...
            throws Exception
    {
        ...
        NomsClient client = new NomsClient(new NomsConfig().setMetadata(metadataUri), CATALOG_CODEC);
        metadata = new NomsMetadata(new NomsConnectorId(CONNECTOR_ID), client);
    }
    ...
    public void testGetColumnHandles()
    {
        ...
        assertEquals(metadata.getColumnHandles(SESSION, NUMBERS_TABLE_HANDLE), ImmutableMap.of(
                "text", new NomsColumnHandle(CONNECTOR_ID, "text", createUnboundedVarcharType(), 0),
                "value", new NomsColumnHandle(CONNECTOR_ID, "value", BIGINT, 1)));

        // unknown table
        try {
            metadata.getColumnHandles(SESSION, new NomsTableHandle(CONNECTOR_ID, "unknown", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
        try {
            metadata.getColumnHandles(SESSION, new NomsTableHandle(CONNECTOR_ID, "example", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
    ...
    public void getTableMetadata()
    {
        ...
        assertNull(metadata.getTableMetadata(SESSION, new NomsTableHandle(CONNECTOR_ID, "unknown", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new NomsTableHandle(CONNECTOR_ID, "example", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new NomsTableHandle(CONNECTOR_ID, "unknown", "numbers")));
    }
    ...
    {
        assertEquals(metadata.getColumnMetadata(SESSION, NUMBERS_TABLE_HANDLE, new NomsColumnHandle(CONNECTOR_ID, "text", createUnboundedVarcharType(), 0)),
                new ColumnMetadata("text", createUnboundedVarcharType()));
        ...
        // NomsTableHandle and NomsColumnHandle passed in.  This is on because
