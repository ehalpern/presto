        ...
        implements ConnectorMetadata
{
    ...
    private final com.facebook.presto.noms2.NomsClient nomsClient;
    ...
    public NomsTableHandle getTableHandle(ConnectorSession session, SchemaTableName tableName)
    {
        ...
        NomsTable table = nomsClient.getTable(tableName.getSchemaName(), tableName.getTableName());
        if (table == null) {
            return null;
        }
    ...
    {
        NomsTableHandle nomsTableHandle = (NomsTableHandle) table;
        checkArgument(nomsTableHandle.getConnectorId().equals(connectorId), "tableHandle is not for this connector");
        SchemaTableName tableName = new SchemaTableName(nomsTableHandle.getSchemaName(), nomsTableHandle.getTableName());
    ...
    public List<SchemaTableName> listTables(ConnectorSession session, String schemaNameOrNull)
    {
        ...
        if (schemaNameOrNull != null) {
            ...
            schemaNames = nomsClient.getSchemaNames();
        }

        ImmutableList.Builder<SchemaTableName> builder = ImmutableList.builder();
        for (String schemaName : schemaNames) {
            for (String tableName : nomsClient.getTableNames(schemaName)) {
    ...
    public Map<String, ColumnHandle> getColumnHandles(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        NomsTable table = nomsClient.getTable(nomsTableHandle.getSchemaName(), nomsTableHandle.getTableName());
        if (table == null) {
            throw new TableNotFoundException(nomsTableHandle.toSchemaTableName());
        }

        ImmutableMap.Builder<String, ColumnHandle> columnHandles = ImmutableMap.builder();
        int index = 0;
        for (ColumnMetadata column : table.getColumnsMetadata()) {
            columnHandles.put(column.getName(), new NomsColumnHandle(connectorId, column.getName(), column.getType(), index));
            index++;
        }
